    ...
    // : tcpClient_(loop, serverAddr)
{
    ...
    //     [this](TcpConnectionPtr conn) { onConnection(std::move(conn)); });

    // tcpClient_.setMessageCallback(
    //     [this](const TcpConnectionPtr& conn, Buffer* buff, Timestamp tm)
...
// {
//     reconnectConfig_.maxRetries    = config.maxRetries;
//     reconnectConfig_.baseBackoffMs = config.baseBackoffMs;
//     reconnectConfig_.maxBackoffMs  = config.maxBackoffMs;
//     reconnectConfig_.jitterFactor  = std::min(std::max(config.jitterFactor, 0.0), 0.5);
// }
