...
void EtcdDiscovery::fetchAndNotify(const std::string& serviceName) {
    ...
    try {
      ...
      if (!resp.is_ok()) {
        LOG_ERROR << "EtcdDiscovery: ls failed for " << prefix << ": "
                  << resp.error_message();
        return;
      }
    ...
    if (changed) {
        ...
        LOG_INFO << "EtcdDiscovery: endpoint changed for " << svc << " ("
...
Endpoint EtcdDiscovery::jsonToEndpoint(const std::string& json) {
  Endpoint ep;
	...
	// 解析 host
	auto hostPos = json.find("\"host\"");
	if (hostPos != std::string::npos) {
		auto start = json.find('\"', hostPos + 7) + 1;
		auto end = json.find('\"', start);
		ep.host = json.substr(start, end - start);
	}

	// 解析 port
	auto portPos = json.find("\"port\"");
	if (portPos != std::string::npos) {
		auto start = json.find(':', portPos) + 1;
		auto end = json.find_first_of(",}", start);
		ep.port = static_cast<uint16_t>(
			std::stoi(json.substr(start, end - start)));
	}

	// 解析 weight
	auto weightPos = json.find("\"weight\"");
	if (weightPos != std::string::npos) {
		auto start = json.find(':', weightPos) + 1;
		auto end = json.find_first_of(",}", start);
		ep.weight = static_cast<uint16_t>(
			std::stoi(json.substr(start, end - start)));
	} else {
		ep.weight = 1;
	}
